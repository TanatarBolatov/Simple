        ...
        <div className="min-h-screen bg-gray-50 flex flex-col">
            ...
            <div className="flex-1 p-4">
                ...
                <div className="space-y-4">
                    ...
                    <button
                        ...
                        className={`w-full flex items-center p-4 rounded-xl pulse-button ${method === 'kaspi' ? 'active' : ''}`}
                        style={{ '--color': '#ef4444', '--hover': '#ef4444' } as React.CSSProperties}
                    >
                        <div className="w-10 h-10 bg-red-500 rounded-lg flex items-center justify-center text-white font-bold text-xs mr-4 shrink-0">QR</div>
                        <span className="font-semibold flex-1 text-left">Kaspi QR</span>
                        {method === 'kaspi' && <div className="w-4 h-4 rounded-full bg-white"></div>}
                    </button>

                    {/* Card - Dark */}
                    <button
                        onClick={() => setMethod('card')}
                        className={`w-full flex items-center p-4 rounded-xl pulse-button ${method === 'card' ? 'active' : ''}`}
                        style={{ '--color': '#1f2937', '--hover': '#1f2937' } as React.CSSProperties}
                    >
                        <div className="w-10 h-10 bg-gray-800 rounded-lg flex items-center justify-center text-white mr-4 shrink-0">
            ...
            <div className="p-4 bg-white border-t border-gray-100">
                <button
                    ...
                    className="w-full bg-brand-green text-white font-bold h-14 rounded-xl flex items-center justify-center text-lg shadow-lg shadow-brand-green/20 active:scale-[0.98] transition-transform disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed"
                >
                    {isProcessing ? (
